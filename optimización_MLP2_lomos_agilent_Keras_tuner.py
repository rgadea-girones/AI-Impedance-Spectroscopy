...
def create_model(hp):
    ...
    hp_units1 = hp.Int('units1', min_value=50, max_value=100, step=5)
    hp_units2 = hp.Int('units2', min_value=20, max_value=50, step=5)
    model.add(Dense(hp_units1, activation='tanh', activity_regularizer='L2'))
    model.add(Dense(hp_units2, activation='tanh'))
...
tuner = kt.BayesianOptimization(create_model,
                     ...
                     directory='my_dir_optimizacion',
                     project_name='kt_bayesian_nuevo_linux')


# perform the optimization
tuner.search(X_train_Normalizado, y_train_Normalizado, epochs=1000, validation_data=(X_test_def,y_test_def))# 
#get the optimal hyperparameters
best_hps=tuner.get_best_hyperparameters(num_trials=1)[0]

# print out 

#nodos1=best_hps.get('units1')  
nodos1=best_hps.get('units1')
nodos2=best_hps.get('units2')

print(nodos1)
print(nodos2)

# get the optimized model
model_optimo = tuner.hypermodel.build(best_hps)
...
experimento="LOMOS_Agilent_entradas_{}_dense1_{}_dense2_{}_clases_{}_loss_{}_lr_{}_algoritmo_{}".format(slice_str,nodos1,nodos2,numero_clases,lossfunction,factor_aprendizaje,algoritmo)
...
model_optimo.fit(X_train_def, y_train_def, epochs=numero_epochs, batch_size=100, callbacks=[tensorboard_callback,early_stop,reduce_lr], validation_data=(X_val_def, y_val_def))
# Final evaluation of the model 
scores = model_optimo.evaluate(X_test_def, y_test_def, verbose=0)
print("Accuracy: %.2f%%" % (scores[1]*100))

y_pred = model_optimo.predict(X_test_def)
#y_pred1=y_pred[:,-1]
...
model_optimo.save('modelos\modelo_optimo_{}_{}.h5'.format(experimento,datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))) 